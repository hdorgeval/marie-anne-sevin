...
function addPaswwordVisibilityToggler(): void {
	try {
		const netlifyFrames = Array.from(
			document.querySelectorAll("iframe#netlify-identity-widget"),
		);
		const iframe: HTMLIFrameElement | null =
			netlifyFrames.pop() as HTMLIFrameElement;
		...
		const fragment = buildPasswordTogglerFragmentFor(
			passwordInput,
			iframeDocument,
		);
...
export const userContextInfo: UserInfo = {
	...
	login: () => {
		...
		const hamburgerMenuCloseButton: HTMLButtonElement | null =
			document?.querySelector("button#hamburger-close-button");
