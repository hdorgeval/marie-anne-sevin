...
export const MultiLinesQuote: FC<MultiLinesQuoteOwnProps> = ({
	sentences,
	author,
}) => {
				...
				{sentences.map((s, index) => (
					<p key={s}>
						...
						{index > 0 && index < lastIndex && (
							<span className="fs-5 fst-italic ps-4">{s}</span>
						)}
						{index === lastIndex && (
							<>
								<span className="fs-5 fst-italic ps-4">
									{sentenceWithoutLastWord(words(s))}
								</span>
